...
public class RegistrySchemaService implements Serializable {
	...
	public static List<String> DEFAULT_STORE_FIELD_LIST = Arrays.asList(WorkflowKernel.UNIQUEID, "type", "$readaccess",
			"$taskid", "$writeaccess", "$workflowsummary", "$workflowabstract", "$workflowgroup", "$workflowstatus",
			"$modified", "$created", "$modelversion", "$lasteventdate", "$creator", "$editor", "$lasteditor", "$owner",
			"namowner", "$api");
