...
/**
 * This service is a singleton EJB which registers the Imixs-Microservice during startup at a
 * Imixs-Registry. The Self-Registration is only performed if the imixs.property
 * 'imixs.registry.service' is set.
 * <p>
 * If the service has successfully registered at a Imixs-Registry, the service starts a timer to
 * update the registry in an interval defined by the property 'imixs.registry.intervall' in ms. This
...
public class RegistrySelfRegistrationService implements Serializable {
  ...
  /**
   * On startup we just initialize a new Timer running each 10 seconds to auto register on a given
   * imixs-registry service api endpoint.
   * <p>
   * 
  ...
  void init() {
    ...
    if (!registryAPI.isEmpty()) {
      ...
      if (findTimer() == null) {
        logger.finest(
            "......create new timer: " + timerID + " - timer intervall=" + registryInterval + "ms");
  ...
  private void registerMicroservice() {
    ...
    try {
      ...
      logger.severe(
          "Unable to register service at Imixs-Registry: " + registryAPI + " - " + e.getMessage());
