...
/**
 * The DiscoveryService supports methods to start a new process instance based on a given business
 * event. The service discovery process automatically selects a matching workflow service based on
 * the data provided with the business event and starts a new process instance.
 * <p>
 ...
 * <p>
 * During the discovery process the service sends discoveryEvents which can be used by an observer
 * CDI bean to intercept the life cycle of the process. In case an observer returns a service
 * endpoint by itself the discovery process is stopped and the provided api endpoint is returned
 * without evaluating any business rules.
 * 
 * @author Ralph Soika
 * @version 1.0
 */
...
public class DiscoveryService {
   ...
   * <p>
   * During the discovery process the service sends discoveryEvents which can be used by an observer
   * CDI bean to intercept the life cycle of the process.
   * <p>
   * An observer may select a service endpoint by itself. In this case the discovery process is
   * stopped and the provided api endpoint is returned.
   * 
   * @param businessEvent
   */
  public void discoverService(ItemCollection businessEvent) {
    ...
    try {
      ...
      if (!found && discoverServiceByRule(businessEvent)) {
        found = true;
        if (debug) {
          logger.fine("......service discovery by rule completed in "
              + (System.currentTimeMillis() - l) + "ms");
        }
      }
   ...
   * <p>
   * In case no model version was provided or no appropriate service matches the business event, the
   ...
   */
  private boolean discoverServiceByModelVersion(ItemCollection businessEvent)
      throws ModelException {
    String service = null;
    ...
    if (service != null && !service.isEmpty()) {
      ...
      /*
       * We assume that the $taskid and $eventid is correctly set and did not perform a validation
       * here for performance reasons. In case the taskID or eventId is invalid the endpoint will
       * throw an exception. This is a configuration issue.
       */
      return true;
    }
    ...
    if (versions.size() > 1) {
      ...
      for (String version : versions) {
        ...
        if (task != null) {
          ItemCollection event =
              model.getEvent(businessEvent.getTaskID(), businessEvent.getEventID());
          if (event != null) {
            // match!
      ...
      logger.warning("Invalid model expression '" + modelVersion
          + "' did not match any registered model version!");
   ...
   * @throws ModelException
   */
  private boolean discoverServiceByWorkflowGroup(ItemCollection businessEvent)
      throws ModelException {
    String service = null;
   ...
   * <p>
   * In case a service was found, the method set the item $api, $modelversion, $eventid and returns
   * true.
   * <p>
   * In case no model version was provided or no appropriate service matches the business event, the
   * method returns false.
    ...
    for (BPMNModel model : models) {
      ...
      try {
        ...
        String message = "Discover Business Rule failed - $modelversion=" + model.getVersion()
            + " ▷ " + businessEventClone.getTaskID() + "→" + businessEventClone.getEventID()
            + " ERROR: " + e.getMessage();
      ...
      if (!task.getItemValueBoolean("endTask")) {
        ...
        if (debug) {
          logger.fine(
              "......discovered Service by rule in " + (System.currentTimeMillis() - l) + "ms");
        }
        return true;
      }
    }
