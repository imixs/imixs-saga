...
/**
 * The UpdateService is a singleton EJB which maintains a solr index based on event log entries
 * provided by a registered Imixs-Microservice. At startup the service starts a timer service to
 * update the index in the 'imixs.index.intervall' (defined in ms). This feature will ensure that
 * the Imixs-Registry holds an updated solr index
 * <p>
 * The environment variable 'solr.api' defines the solr service endpoint and lucene core. If no
 * endpoint is defined (default) no index will be written.
 * <p>
 * The service reads all enventLogEntires from all registered Imixs-Microserivces and delegates the
 * index update to the class SolrUpdateervice.
 * 
...
public class UpdateService implements Serializable {
  ...
  void init() {
    ...
    if (!api.isEmpty()) {
      ...
      if (findTimer() == null) {
        if (debug) {
          logger.finest(
              "......create new timer: " + timerID + " - timer intervall=" + indexInterval + "ms");
        }
  ...
  /**
   * This method flush the event log for all imixs-microservices registered at the experience
   * registry.
   */
  public synchronized void updateIndex() {
    if (!api.isEmpty()) {
      ...
      if (services != null && services.size() > 0) {
        ...
        if (debug) {
          logger
              .fine("...full index update completed in " + (System.currentTimeMillis() - l) + "ms");
        }
      }
    }
  }
  ...
  /**
   * On the timeout event we flush the event log for all imixs-microservices registered at the
   * experience registry.
   * 
   */
  @Timeout
  ...
  /**
   * This helper method reads the imixs-registry.index topics for a given Imixs-Microservice
   * instance.
   * 
   * @return
   */
  private List<ItemCollection> loadEventLog(EventLogClient client) {
    ...
    try {
      ...
      eventLogEntries =
          client.searchEventLog(EVENTLOG_TOPIC_INDEX_ADD, EVENTLOG_TOPIC_INDEX_REMOVE);
    } catch (RestAPIException e) {
      logger
          .severe("Unable to read eventlog from: " + client.getBaseURI() + " - " + e.getMessage());
    }
   ...
   * <p>
   * The method creates two distinct collections containing the workitems to be added into the index
   * and a collection containing. This is to reduce the rest api calls into solr. Only one update
   * and one delete api call is processed
   * 
   * 
   * <p>
   * After the solr index was updated, the method deletes the eventLog entries from the
   * Imixs-Microservice instance vie the imixs-microserivce Rest API.
   * 
   * @param eventLog entries to be added or removed.
