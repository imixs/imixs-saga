...
public class RegistryService {
  ...
  private Map<String, List<BPMNModel>> serviceRegistry =
      new ConcurrentHashMap<String, List<BPMNModel>>();
   ...
   * <p>
   * The method verifies if the same model version already exists. If so, we remove the deprecated
   * version and replace it with the current
   * 
  ...
  /**
   * This method returns a service endpoint for a given model version or null if no service with
   * this model version is registered.
   * 
   * @param version
   * @return
   */
